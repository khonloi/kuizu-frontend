...
export const mockApi = {
  ...
  // Flashcards
  getPublicFlashcardSets: async () => { return mockFlashcardSets.filter(s => s.visibility === "PUBLIC"); },
  getMyFlashcardSets: async (userId = "u1-uuid-1234-5678") => { 
    return mockFlashcardSets.filter(s => s.owner.userId === userId); 
  },
  getFlashcardSetById: async (setId) => { return mockFlashcardSets.find(s => s.setId === parseInt(setId)); },
  ...
  getMyFolders: async (userId = "u1-uuid-1234-5678") => { 
    return mockFolders.filter(f => f.owner.userId === userId); 
  },
  ...
  getClassDetails: async (classId) => {
    ...
    return { ...clazz, members, materials: [] };
  },
