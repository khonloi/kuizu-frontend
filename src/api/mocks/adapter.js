...
export const setupMockAdapter = (api) => {
  api.defaults.adapter = async (config) => {
    ...
    console.log(
      `[Mock API] Intercepted: ${config.method.toUpperCase()} ${url}`,
    );
    ...
    try {
      ...
      // User / Profile
        ...
        data = await mockApi.updateProfile(JSON.parse(config.data));
      // Flashcard Sets
      else if (url.includes("/flashcard-sets/my"))
        data = await mockApi.getMyFlashcardSets();
      ...
      // Flashcards
        ...
        data = setIdMatch
          ? await mockApi.getFlashcardsBySetId(setIdMatch[1])
          : [];
      }

      // Folders
      else if (url.includes("/folders/me")) data = await mockApi.getMyFolders();
      else if (url.includes("/folders/public"))
        data = await mockApi.getPublicFolders();
      ...
      else if (url.includes("/classes/public"))
        data = await mockApi.getPublicClasses();
      else if (url.includes("/classes/")) {
        const classIdMatch = url.match(/\/classes\/(\d+)/);
        if (classIdMatch) {
          if (url.includes("/join")) {
            data = await mockApi.joinClass(
              classIdMatch[1],
      ...
      // Study / Quiz
      else if (url.includes("/study/quiz/submit"))
        data = await mockApi.submitQuiz(JSON.parse(config.data));
      ...
      return Promise.reject({
        response: {
          data: {
            message:
              error.message || error.response?.data?.message || "Mock error",
          },
